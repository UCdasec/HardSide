...
def computeRankingCurve(preds, key, plaintext, targetByte, rankRoot, leakageModel, traceNumMax, fileName,
    ...
    :param plaintextAttack Plaintext attack mode
    :return: None
    """
    ...
    plaintext    = plaintext[:, targetByte] if not plaintextAttack else plaintext
    resultQueue  = Queue()
    ...
    for i in range(cpu_count() if numberOfAttacksToAverage > cpu_count() else numberOfAttacksToAverage):  # Ease memory usage for increased speed up
